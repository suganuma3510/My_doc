...
マイクロサービスは、1サービスを1プロセスとして稼働させる仕組み。  
サービス同士はAPIによって最小限の連携をもつのみで、基本的にはそれぞれ独立している。  
そのため、設計にあたってはそれぞれのサービスに特化した手法をとることができる。  

メリット
- 障害時は1つのサービスごとに影響をチェックできる
- 部分的なスケーリングが可能
- メンテナンスやデプロイなどが容易
...
### クイックストリーム
Webサイトの閲覧者が訪問から離脱までの間にどのページをどのような順序で遷移したかを記録したデータのこと。
...
### ローリングアップデート
ソフトウェアの更新、入れ替えの方法の一つで、稼働中のシステムを完全には停止させずに徐々に新しいものに入れ替えていく方式のこと。  
同じ構成のコンピュータが複数ある場合に、システムの稼働状態を維持しながら1台ずつ順に更新を行う。
