...
- [GitHub](#github)
  - [ログイン状態の確認](#ログイン状態の確認)
  - [ログイン](#ログイン)
  - [アカウントの切り替え](#アカウントの切り替え)
  - [組織の確認](#組織の確認)
  - [リポジトリ一覧](#リポジトリ一覧)
  - [ワークフローの確認](#ワークフローの確認)
  - [GitHub Actionsのワークフローの実行結果を確認](#github-actionsのワークフローの実行結果を確認)
  - [pull requestを作成](#pull-requestを作成)
...
#### 空コミット
```
git commit --allow-empty -m "空デプロイ実施のため空コミットする"
```
...
#### push先のリポジトリを変更
```
git remote set-url origin git@github.com:MikotoSuganuma/media_app.git
```
...
git config user.email "<YUOR_EMAIL>"
```
...
#### pull requestを作成
```
gh pr create --base develop --head feature/ --title "タイトル" --body "内容"

# プルリクエストの作成（HEREDOCを使用）
gh pr create --title "feat: 〇〇機能の追加" --body "$(cat <<'EOF'
## 概要
